...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<div class="row justify-content-center align-items-center" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		...
		<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
			<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
				<a href="<?echo $arItem["DETAIL_PAGE_URL"]?>" class="trainingName"><b><?echo $arItem["NAME"]?></b></a><br />
			<?else:?>
		...
		<?endif?>
			<div class="trainingDate">
                <div class="DateStamp">
                    <?php
                    $date = $arItem["DISPLAY_PROPERTIES"]["date_training"]["DISPLAY_VALUE"];
                         echo '<b>'.strtolower(FormatDate("d F", MakeTimeStamp($date))).'</b><br><div class="smallDate">'.$date.'</div>';
                    ?>
                </div>
                <div class="trainingAnchor">
                    <b><?=$arItem["PROPERTIES"]["anchor_training"]["VALUE"];?></b><br>
                    <?=$arItem["PROPERTIES"]["profAnchor_training"]["VALUE"];?>
                </div>
			</div><br />
		<? //endforeach;?>
		<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
			<a href="<?echo $arItem["DETAIL_PAGE_URL"]?>" class="trainingContinue"><b>Подробнее</b></a>
		<?endif;?>
		</div>
	</div>
<?endforeach;?>
